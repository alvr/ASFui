...
namespace ASFui
{
    ...
    public class ASFProcess
    {
        ...
        {
            if (ASFEnded || e == null || sender == null || e.Data == null) return;

            var inputData = e.Data.Trim();
            if (inputData.EndsWith("\"android:\"):", StringComparison.OrdinalIgnoreCase) ||
                inputData.EndsWith("login:", StringComparison.OrdinalIgnoreCase) ||
                inputData.EndsWith("+1234567890):", StringComparison.OrdinalIgnoreCase) ||
                inputData.EndsWith("mobile:", StringComparison.OrdinalIgnoreCase) ||
                inputData.EndsWith("email:", StringComparison.OrdinalIgnoreCase) ||
                inputData.EndsWith("PIN:", StringComparison.OrdinalIgnoreCase) ||
                inputData.EndsWith("app:", StringComparison.OrdinalIgnoreCase) ||
                inputData.EndsWith("hostname:", StringComparison.OrdinalIgnoreCase))
            {
            ...
            var match = Regex.Match(e.Data, @".*Key: (.*) \| Status: .*(NoDetail|DuplicateActivationCode|BadActivationCode|AlreadyPurchased|RateLimited)");
            if (match.Success)
            {
                    ...
                    ("RateLimited".Equals(type) && Properties.Settings.Default.ClearCooldown))
                {
                    ...
                    {
                        _asf.tbInput.Text = Regex.Replace(_asf.tbInput.Text.Replace(key, ""), @"\s+",
                            Environment.NewLine);
